  ...
  private async handleTextMessage(message: any, whatsappAccountId: string) {
    ...
    if (conversation.aiEnabled) {
      ...
      try {
        ...
        if (!apiKey) {
          console.error("No Gemini API Key configured for AI automation.");
          return;
        }
        ...
        // Build prompt
        let promptContext = "You are a helpful customer service assistant for a business on WhatsApp. Reply politely and concisely.\n\nConversation History:\n";
        recentMessages.forEach((msg: any) => {
          promptContext += `${msg.direction === 'INCOMING' ? 'Customer' : 'Assistant'}: ${msg.content || '[Media]'}\n`;
        });
        ...
        if (aiResponseText) {
          ...
          // Save outgoing AI message to database
          await prisma.message.create({
            data: {
              conversationId: conversation.id,
              direction: 'OUTGOING',
        ...
        console.error("AI Generation Error in Webhook:", aiError);
      }
    }
  }
