          ...
          {pricingPlans.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl p-7 flex flex-col transition-all ${
                plan.highlighted
                  ? "bg-[#2563eb] border border-[#a855f7]/50 shadow-lg"
            ...
            >
              ...
              <div className="mb-6">
                <h3 className={`text-xl font-bold mb-2 ${plan.highlighted ? "text-white" : "text-[#0f172a]"}`}>
                  {plan.name}
                </h3>
                <p className={`text-sm ${plan.highlighted ? "text-[#1e3a8a]" : "text-[#64748b]"}`}>
                  {plan.description}
                </p>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-end gap-1">
                  <span className={`text-4xl font-extrabold ${plan.highlighted ? "text-white" : "text-[#0f172a]"}`}>
                    ₹{(annual ? plan.price.yearly : plan.price.monthly).toLocaleString("en-IN")}
                  </span>
                  <span className={`text-sm mb-1.5 ${plan.highlighted ? "text-[#1e3a8a]" : "text-[#64748b]"}`}>
                    /mo
                  </span>
                </div>
                {annual && (
              ...
              <ul className="space-y-3 flex-1 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5">
                    ...
                    />
                    <span className={`text-sm ${plan.highlighted ? "text-[#1e3a8a]" : "text-[#475569]"}`}>
                      {f}
                    </span>
                  </li>
                ))}
